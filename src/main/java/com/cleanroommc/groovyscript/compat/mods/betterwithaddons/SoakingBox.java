...
public class SoakingBox extends StandardListRegistry<CherryBoxRecipe> {
    ...
    public Collection<CherryBoxRecipe> getRecipes() {
        return CraftingManagerSoakingBox.instance().getRecipes();
    }

    @MethodDescription(example = @Example("item('betterwithaddons:bamboo')"))
    ...
    @MethodDescription(example = @Example("item('betterwithaddons:japanmat:8')"))
    ...
    public static class RecipeBuilder extends AbstractRecipeBuilder<CherryBoxRecipe> {
        ...
        public String getErrorMsg() {
            return "Error adding Better With Addons Soaking Box recipe";
        }
