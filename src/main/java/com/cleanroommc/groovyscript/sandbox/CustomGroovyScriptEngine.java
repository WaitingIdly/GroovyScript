...
public class CustomGroovyScriptEngine implements ResourceConnector {
    ...
    static boolean shouldDeleteCache(JsonObject json) {
        return json.get("version").getAsInt() != CACHE_VERSION ||
               json.has("java") && !json.get("java").getAsString().equals(VMPlugin.getJavaVersion()) ||
               json.has("packId") && !json.get("packId").getAsString().equals(GroovyScript.getRunConfig().getPackId()) ||
               json.has("packVersion") && !json.get("packVersion").getAsString().equals(GroovyScript.getRunConfig().getVersion());
    }
    ...
    void writeIndex() {
        ...
        json.addProperty("packId", GroovyScript.getRunConfig().getPackId());
        json.addProperty("packVersion", GroovyScript.getRunConfig().getVersion());
